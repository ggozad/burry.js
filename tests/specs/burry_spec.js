  ...
  describe("burry.js Storage", function() {
    ...
    describe("Static methods", function() {
      ...
      it("flushes expired key/values from all stores", function() {
        ...
        Burry.flushExpired();
        expect(
          localStorage.getItem(burryfoo._internalKey("expired1"))
        ).toBeNull();
        expect(
          localStorage.getItem(burryfoo._expirationKey("expired1"))
        ).toBeNull();
        expect(
          localStorage.getItem(burryfoo._internalKey("expired2"))
        ).toBeNull();
        expect(
          localStorage.getItem(burryfoo._expirationKey("expired2"))
        ).toBeNull();
        expect(burryfoo.get("not-expired")).toBeDefined();
        expect(
          localStorage.getItem(burrybar._internalKey("expired1"))
        ).toBeNull();
        expect(
          localStorage.getItem(burrybar._expirationKey("expired1"))
        ).toBeNull();
        expect(
          localStorage.getItem(burrybar._internalKey("expired2"))
        ).toBeNull();
        expect(
          localStorage.getItem(burrybar._expirationKey("expired2"))
        ).toBeNull();
    ...
    describe("Instance methods", function() {
      ...
      it("allows to set a default ttl", function() {
        ...
        expect(parseInt(localStorage.getItem("akey-_burry_exp_"), 10)).toEqual(
          Burry._mEpoch() + 10
        );
      });
      ...
      it("applies correctly the namespace on the keys on construction", function() {
        ...
        expect(nsburry._isExpirationKey("foo-_burry_exp_testing")).toEqual(
          "foo"
        );
      ...
      it("returns all expirable keys", function() {
        ...
        burry.set("expirable2", { foo: "bar" }, 20);
        burry.set("non-expirable", { foo: "bar" });
        expect(burry.expirableKeys()).toEqual({
          expirable1: 10,
          expirable2: 20
        });
      });
      ...
      it("flushes expired key/values", function() {
        ...
        expect(
          localStorage.getItem(burry._expirationKey("expired1"))
        ).toBeNull();
        expect(localStorage.getItem(burry._internalKey("expired2"))).toBeNull();
        expect(
          localStorage.getItem(burry._expirationKey("expired2"))
        ).toBeNull();
        ...
        while (true) {
          try {
            ...
            localStorage.setItem(
              burry._internalKey(key),
              JSON.stringify(biggie)
            );
